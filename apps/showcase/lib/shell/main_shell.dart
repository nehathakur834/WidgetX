    ...
    return PopScope(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          centerTitle: false,
          actions: [
            _ScreenFavoriteButton(path: path),
            _FavoritesButton(),
            _ThemeToggle(),
...
/// Returns a human-readable title for the given GoRouter path.
String _titleForPath(String path) {
  ...
  // Fallback for template sub-routes not in the nav list
...
// ── Per-screen favorite button ────────────────────────────────────────────────

/// Shows a heart icon for screens that have a known favorites ID.
/// The ID is the path without the leading slash (e.g. '/charts' → 'charts').
class _ScreenFavoriteButton extends ConsumerWidget {
  const _ScreenFavoriteButton({required this.path});
  final String path;

  // Routes that support the favorites toggle
  static const _favoriteIds = {
    '/foundations': 'foundations',
    '/buttons': 'buttons',
    '/inputs': 'inputs',
    '/cards': 'cards',
  ...
  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final id = _favoriteIds[path];
    if (id == null) return const SizedBox.shrink();
    final isFav = ref.watch(favoritesProvider.select((s) => s.contains(id)));
    return IconButton(
